...
const LRSTracker = () => {
  ...
  const fetchQQQData = async () => {
    try {
      ...
      const qqqResponse = await fetch(
        `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=QQQ&outputsize=full&apikey=${API_KEY}`,
      );

      if (!qqqResponse.ok) {
        throw new Error("Failed to fetch QQQ data from Alpha Vantage");
      }
      ...
      if (qqqJson.Note) {
        throw new Error(
          "API rate limit reached. Please wait a minute and try again, or use your own API key.",
        );
      }

      if (!qqqJson["Time Series (Daily)"]) {
        throw new Error(
          "Invalid response from Alpha Vantage. Please try again.",
        );
      }
      ...
      const tqqqResponse = await fetch(
        `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=TQQQ&outputsize=full&apikey=${API_KEY}`,
      );

      if (!tqqqResponse.ok) {
        throw new Error("Failed to fetch TQQQ data from Alpha Vantage");
      }
      ...
      if (!tqqqJson["Time Series (Daily)"]) {
        throw new Error(
          "Invalid TQQQ response from Alpha Vantage. Please try again.",
        );
      }
      ...
      calculateSignal(qqqPrices);
      calculatePerformance(qqqPrices, tqqqPrices);
  ...
  const calculatePerformance = (qqqPrices, tqqqPrices) => {
    // Find common date range
    const qqqDates = new Set(qqqPrices.map((p) => p.date));
    ...
    const commonDates = qqqPrices
      .filter((p) => tqqqDates.has(p.date))
      .map((p) => p.date)
      .sort();

    // Start simulation from 1 year ago or available data
    const oneYearAgo = new Date();
    oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);

    const simDates = commonDates.filter((date) => new Date(date) >= oneYearAgo);

    if (simDates.length < 200) {
      console.warn("Not enough data for full 200-day SMA");
      setPerformanceData([]);
      return;
    }
    ...
    const performance = simDates.map((date, idx) => {
      ...
      // Calculate 200-day SMA using all available data up to this point
      const allPricesUpToDate = qqqPrices.filter(
        (p) => new Date(p.date) <= new Date(date),
      );
      const sma = calculateSMA(allPricesUpToDate, 200);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Using real historical data for QQQ and TQQQ from Yahoo Finance.
